...
Run equivalent no-retry/format-retry/semantic-retry/best-of policies against real provider, OSS, or HPC rows produced by the same hard nested suite. Those artifacts must use `provider`, `oss`, or `hpc` classifications and must include trace coverage, report, retry audit, triage, and gate decisions.
