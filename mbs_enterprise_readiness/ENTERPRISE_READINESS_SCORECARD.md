...
|---|---|---|---|
| Gate 1 — Install/DX | Partial pass, improved | Clean Windows audit venv verified again on 2026-05-18; fresh wheel install proof passes locally; CI is configured and regression-guarded for Ubuntu, Windows, and macOS; CI now records non-secret environment manifests; compatibility matrix documented | Downloaded remote non-Windows CI artifact evidence still needed |
...
| Gate 9 — CI/regression | Partial pass, improved | Full local repository suite passes: `212 passed, 10 skipped`; focused P0 pack passes: `130 passed`; CI artifact assertion now requires non-secret environment manifests; empty-result and compare `NO_MATCH` false-pass guards covered; workflow configured and tested for Ubuntu/Windows/macOS; local release gates pass for package inspection, fresh install, public artifact classification, release-blocker disposition structure, and Leonardo evidence helper contracts | Remote CI matrix execution artifacts still needed |
