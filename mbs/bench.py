...
def _mock_value(name: str, spec: dict[str, Any], input_text: str) -> Any:
    ...
    if enum:
        ...
        if name in {"decision", "action", "recommendation"} and {
            "APPROVE",
            "STEP_UP_AUTH",
            "HOLD_REVIEW",
            "BLOCK",
        }.issubset(set(enum)):
            return _mock_fintech_decision(upper)
        if name in {"risk_level", "priority", "severity"} and {
            "LOW",
            "MEDIUM",
            "HIGH",
            "CRITICAL",
        }.issubset(set(enum)) and "TXN-" in upper:
            return _mock_fintech_risk_level(upper)
        for value in enum:
    ...
    if t == "number":
        if name == "risk_score":
            level = _mock_fintech_risk_level(input_text.upper())
            return {"LOW": 12.0, "MEDIUM": 48.0, "HIGH": 78.0, "CRITICAL": 98.0}[level]
    ...
    if t == "array":
        ...
        minimum = int(spec.get("minItems") or 0)
        if minimum <= 0:
            return []
        return [_mock_value(name.rstrip("s") or "item", item_spec, input_text) for _ in range(minimum)]
    if t == "object":
        return {
    ...
    if isinstance(pattern, str):
        patterned = _mock_pattern_value(name, pattern, input_text)
...
def _mock_pattern_value(name: str, pattern: str, input_text: str) -> str | None:
    if pattern == "^TXN-[0-9]{5}$":
        match = re.search(r"TXN-\d{5}", input_text.upper())
        return match.group(0) if match else "TXN-00000"
    return None


def _mock_fintech_risk_level(upper_input: str) -> str:
    if "SANCTIONS" in upper_input and "NO SANCTIONS" not in upper_input:
        return "CRITICAL"
    if "VELOCITY SPIKE" in upper_input and "NO VELOCITY SPIKE" not in upper_input:
        return "HIGH"
    if "GEO ANOMALY" in upper_input and "NO GEO ANOMALY" not in upper_input:
        return "HIGH"
    if any(term in upper_input for term in ("NEW COUNTRY", "UNKNOWN", "INTERNATIONAL WIRE", "IMMEDIATE LARGE PAYMENT")):
        return "HIGH"
    if "NEW PAYEE" in upper_input or "ABOVE USUAL" in upper_input or "AMOUNT ANOMALY" in upper_input:
        return "MEDIUM"
    return "LOW"


def _mock_fintech_decision(upper_input: str) -> str:
    return {
        "LOW": "APPROVE",
        "MEDIUM": "STEP_UP_AUTH",
        "HIGH": "HOLD_REVIEW",
        "CRITICAL": "BLOCK",
    }[_mock_fintech_risk_level(upper_input)]
